...
- **Cron runs at UTC midnight**, steps 0–7 in order (archive yesterday → clamp DOM → advance DOY → mark undone DOW/DOM → delete done date/no-ECD tasks after archiving (and mark project tasks whose `todoTaskId` matches a deleted task as done, link cleared) → reorder → reset done calls on the 15th (biweekly) / last day of month (all)), then AI report generation (not a numbered step). All date math is UTC.
