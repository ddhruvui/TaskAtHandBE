...
- **Nothing nightly happens unless something calls `runCron`.** On a long-lived server `scheduleCron()` registers an in-process `node-cron` schedule; on **serverless (Vercel — the deployed target)** no timer survives between requests, so it detects `VERCEL`/`AWS_LAMBDA_FUNCTION_NAME`, returns `false`, and the schedule lives in `vercel.json` → `crons` → `GET /cron/run` at `0 0 * * *`. Never make `scheduleCron` claim a schedule it cannot keep — a timer that silently never fires is how the deployment sat a month with an empty `TaskArchive`, no `InsightStats` snapshot and no insight reports.
- **Cron runs at UTC midnight**, steps 0–11 in order (archive yesterday → advance DOY → mark undone DOW/DOM → delete done date/no-ECD tasks after archiving (and mark project tasks/life events whose `todoTaskId` matches a deleted task as done, link cleared) → delete empty headers and rearrange header priorities → add due life events to the todo (a linked `date` task under an "Events" header, once per year via `lastAddedYear`) → reorder tasks per header → reset done calls on the 15th (biweekly) / last day of month (all) → refresh the `InsightStats` snapshot → summarise-then-prune the archive → trim stored insight reports), then AI report generation (not a numbered step). All date math is UTC.
- **Step 9 (stats snapshot) is AI-free and runs every night**: it recomputes streaks/rates/on-time counts over `TaskArchive` and replaces the single `InsightStats` doc, so it needs no `GEMINI_API_KEY` and ignores `skipInsights`. Its window ends at the real now (archive events carry real insertion times), *not* at the run's `date` override. **The AI report is daily — once per UTC day** (`isInsightDue` in `insightsService`); a second run the same day reports `insightSkipped: "not-due"`. `POST /insights/generate` bypasses the gate but *consumes* the day, so a manual report at noon means the nightly run skips. **`insightGenerated` is always present and a `false` always carries an `insightSkipped` reason** (`opted-out` / `test-env` / `no-api-key` / `vacation` / `not-due` / `no-data` / `error`, the last with `insightError`) — don't go back to omitting the fields, that silence is what made a broken deployment indistinguishable from a quiet night.
...
- **Step 11 bounds `Insights`**: only the newest `INSIGHT_RETENTION_COUNT` (default 100) reports are kept, because `GET /insights/history` caps at `Insight.MAX_HISTORY` and anything older is unreachable. No roll-up — a report is prose derived from `TaskArchive`, and those numbers already survive in `ArchiveSummary`. Retention is floored at `MAX_HISTORY` so it can never make `?limit=100` unservable.
...
| Cron algorithm (steps 0–11)    | `chron.test.js` AND `cron-api.test.js` (API contract side) |
